...
public class SerializerForKryo extends Serializer<MapState<GameObject>> {
    @Override
    ...
    public MapState<GameObject> read(Kryo kryo, Input input, Class<? extends MapState<GameObject>> type) {
        String inName = input.readString();
        List<GameObject> objectList = (List<GameObject>) kryo.readObjectOrNull(input, ArrayList.class);

        return new MapState<>(inName, objectList);
    }
}
